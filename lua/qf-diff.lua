...
local function get_comparison_target(revision)
	if revision then
		...
		if check.code == 0 then
			vim.notify("Comparing to provided revision: " .. revision, vim.log.levels.INFO)
			return revision
		end
		vim.notify(revision .. " is not a valid revision in this repository", vim.log.levels.WARN)
	else
        local check_available_branches = vim.system({ "git", "branch", "-a" }, { text = true }):wait()
        if check_available_branches.code == 0 then
            local input_options = {"Which branch do you want to compare to?"}
            local i = 1
            for branch in check_available_branches.stdout:gmatch("[%S]*\n") do
                print(branch)
                input_options[i] = i .. ". " .. branch
                i = i + 1
            end
            local input = vim.fn.inputlist(input_options)
            local selection = input_options[input]:gsub(input .. ". ", "")
            vim.notify("Comparing to user selection " .. selection, vim.log.levels.INFO)
            return selection
        end
    end
...
---@param revision string The revision to compare against, if ommitted will use the upstream tracking branch
